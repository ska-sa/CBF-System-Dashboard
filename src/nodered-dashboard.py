        ...
        def sensor_polling(self):
            ...
            while True:
                try:
                    ...
                    upload_feeds('jenkins/job_running',
                                 self.job_status.job_running, _host_ip)
                ...
                try:
                    logging.info('Starting katcp connection on %s:%s' %
                                 (_katcp_ip, _katcp_port))
                    ...
                    try:
                        ...
                        logging.info('subarray katcp connection established on %s:%s' % (_katcp_ip,
                                                                                         _array_name))
                        kcp_client = katcp.BlockingClient(
                            _katcp_ip, _array_port)
                        publish.single('instrument/array_port',
                                       _array_port, hostname=_host_ip)
                        logging.debug('Publishing %s to %s' %
                                      (_array_port, _host_ip))
                        publish.single('instrument/array_name',
                                       _array_name, hostname=_host_ip)
                        logging.debug('Publishing %s to %s' %
                                      (_array_name, _host_ip))
                        publish.single('instrument/katcp_ip',
                                       _host_ip, hostname=_host_ip)
                        logging.debug('Publishing %s to %s' %
                                      (_host_ip, _host_ip))
                        kcp_client.setDaemon(True)
                        kcp_client.start()
                        ...
                        reply, informs = kcp_client.blocking_request(katcp.Message.request('sensor-value'),
                                                                     timeout=_timeout)
                        errmsg = 'Failed to issue sensor-value on %s:%s' % (
                            _katcp_ip, _array_port)
                        assert reply.reply_ok(), errmsg
                        kcp_client.stop()
                        f_hosts = [
                            int(i.arguments[-1]) for i in informs if i.arguments[2] == 'n-feng-hosts'][0]
                        x_hosts = [
                            int(i.arguments[-1]) for i in informs if i.arguments[2] == 'n-xeng-hosts'][0]
                    except Exception as e:
                        logging.error(errmsg)
                        logging.exception(str(e))
                        time.sleep(_delay_poll)
                        ...
                        for i in range(f_hosts):
                            hosts_sensors.append(
                                ''.join(['fhost', str(i), '-network-tx-ok']))
                            hosts_sensors.append(
                                ''.join(['fhost', str(i), '-phy-ok']))
                            hosts_sensors.append(
                                ''.join(['fhost', str(i), '-qdr-ok']))
                            hosts_sensors.append(
                                ''.join(['fhost', str(i), '-reorder-ok']))
                            hosts_sensors.append(
                                ''.join(['fhost', str(i), '-lru-ok']))
                            hosts_sensors.append(
                                ''.join(['fhost', str(i), '-network-rx-ok']))
                            hosts_sensors.append(
                                ''.join(['fhost', str(i), '-pfb-ok']))
                        for i in range(x_hosts):
                            hosts_sensors.append(
                                ''.join(['xhost', str(i), '-lru-ok']))
                            hosts_sensors.append(
                                ''.join(['xhost', str(i), '-network-rx-ok']))
                            hosts_sensors.append(
                                ''.join(['xhost', str(i), '-network-tx-ok']))
                            hosts_sensors.append(
                                ''.join(['xhost', str(i), '-phy-ok']))
                            hosts_sensors.append(
                                ''.join(['xhost', str(i), '-qdr-ok']))
                            hosts_sensors.append(
                                ''.join(['xhost', str(i), '-reorder-ok']))
                        ...
                        for _sensor, _value in sensors.iteritems():
                            if re.search('-', _sensor):
                                topic = ''.join(
                                    ['sensor_values/', _sensor.replace('-', '_')])
                            else:
                ...
                time.sleep(_delay_poll)
                logging.info('Sleeping for %s to ease up the cpu' %
                             _delay_poll)
